...
- インタラクティブモード（デフォルト）: GM が Q1 から Q9 まで1問ずつ表示する。必要なら各 Q で最大1回だけ補足質問する。
- batch モード（`--prompt-only`）: Q1 から Q9 を一括表示する。補足質問は行わない。
...
毎回そのキャラの場面に出てくる「物的な印」を1つ教えてください。
持ち物、アクセサリー、癖、香り、音などです。
...
補足質問:
- 入力が空で「おまかせ」表記もない場合。
- 「アクセサリー」「持ち物」だけのように具体物がない場合。

---

### Q4. 表と内の差

他人に見せる顔と、一人の時の違いを教えてください。
...
過去の出来事、癖、習慣、こだわり、抱えていること、何でもOK。
傷でも、特徴でも、職業的なものでも、好きなものでも構いません。
...
2. `./lilia apply-newgame <session> <answers.md>` を実行する。launcher が LLM CLI(codex または claude)を呼んで character YAML を生成する。
3. character YAML 生成後、launcher が `generate_profile_document(answers=..., character_yaml=..., engine=...)` を呼び、AI-driven `profile.md` を生成する。
4. profile generator が `ProfileGenerationError` を返した場合、apply-newgame は hard-fail する。壊れた `profile.md` を保存しない。
5. Codex 自身が character YAML や profile.md を直接書こうとしない。launcher の出力を読む。
...
9. `current/story_spine.md` と `story/relationship_spine.md` は、character YAML生成後に `tools/story/spine_generator.py` でAI駆動生成する。穴埋めテンプレートは使わない。
10. `profile.md` の `Initial Scene Anchors` / `context` / `unspoken` / `everyday anchors` から、`current/scene.md`、`current/event_card.md`、`story/story_deck.md`、`current/hotset.md` などの現在場面用ファイルを初期化する。
11. `current/event_card.md` には Scene Exit / Next Beat を置き、3-5ターン以内にその場しのぎや立ち話だけで停滞せず次beatへ移れるようにする。
12. first scene前に必ず `profile.md` と current/story/hotset 初期状態を読む。
13. profile.md にある具体物、職能、生活、反応、矛盾、禁忌を使って初回sceneを書く。
14. first sceneで名乗る場合は、`lilia_display_name` または `lilia_name` を使う。「私は、リリア」とは名乗らない。
15. 初回sceneでLILIAを完成させず、ユーザーの選択に対する反応を観察する。
16. 色気、身体距離、きわどさは初回sceneにも使ってよいが、報酬、媚び、親密成立済み、攻略達成として扱わない。
17. 近い距離を書く場合は、相互性、境界線、止まれる余地、LILIA本人の主体性と拒否できる余地を同時に残す。
...
- Q3の描写の縛りを profile.描写の縛り / everyday anchors へ直接反映する
...
Newgame Q1-Q9の回答後、初回sceneを出す前に `prompt/style_reference.md` の Light Story Reference Pass を軽く通す。
このpassは Persona Profile Generation Pass の後に行い、`profile.md` の生活、具体物、反応、矛盾を文体・温度へ接続する。

このpassは固定プロットや参照作品の再現ではない。
Q&Aから、ヒロインの基本、見た目、描写の縛り、表と内の差、内面に持っているもの、最初の出会い、主人公の身体・格好・仕事、呼称を抽出する。
...
Newgame Q1-Q9から裏生成した小さな出来事を、ユーザーが今触れる入口、関係に残る賭け、放置時の小さな変化へ変換する。
handlesは選択肢ではなく、自由入力の行動余地として扱う。
`story/story_deck.md` は素材・圧・未回収札、`current/event_card.md` は今のsceneで触れる可視イベントとして分ける。
Q9に避けたい展開がある場合は、event_cardが助け待ち一本道、明白な正解行動、重すぎる事件、甘すぎる成立済み関係へ寄っていないか確認する。
...
Newgame Q1-Q9とGM生成した保留 / 境界線から、`lilia/main/voice.md` へ呼び方、口調、沈黙、第一反応、言わない言葉を保存する。
...
- character YAML とAI profile generatorから作る
- 基礎情報
- appearance
- tone
- personality
...
## story_spine.md / relationship_spine.md のAI生成

Wave 11以降、`current/story_spine.md` と `story/relationship_spine.md` は `./lilia apply-newgame` 内の `tools/story/spine_generator.py` がAI駆動で生成する。
Pythonテンプレートや `{}` 穴埋めで初期化しない。

### 元データ

- Q&Aの回答
- 生成済みの `lilia/main/character.yaml`
- `references/story_pattern_stock.md`（12パターン、観察作品行はプロンプト投入前に除去）
- `references/story_structure_stock.md`（5構造、具体作品例はプロンプト投入前に除去）

### 生成方針

- AIがQ&Aとcharacter YAMLを解釈し、値をそのまま貼らず自然な日本語へ再構成する。
- 構造を1つ、パターンを1-2個だけ選ぶ。3個以上は混線するため不可。
- `story_spine.md` は Main Question / Reveal Ladder / Background Truth / Pressure Direction / Heroine Tie / if ignored / Drift Guard を必ず持つ。
- `relationship_spine.md` は 育てたいテーマ / 最初の摩擦 / LILIA が守るもの / LILIA が避けるもの / ユーザー側に問うこと / 関係が変化する方向 を必ず持つ。
- Background Truth に「未確定: 進行で決める」で逃げず、仮説でよいので具体内容を書く。
- 参考作品の固有名詞、人物名、地名、組織名、設定名を出力へ混入させない。
- 1文ずつ完結させ、`…` で途切れた文やQ1の長文丸写しを出さない。

### validator

生成後は `tools/story/spine_validator.py` で検査する。
作品名literal混入、必須セクション欠落、空欄回避、文崩壊、同一フレーズ反復、Q1の30文字以上の丸写しを検知した場合は最大2回まで再生成する。
3回失敗した場合は `apply-newgame` を失敗させ、壊れたspineを保存しない。
...
- Q2（見た目）→ profile.appearance / profile.body / profile.outfit
- Q3（描写の縛り）→ 「描写の縛り」/ everyday anchors の物的アンカー
- Q4（表と内の差）→ profile.contradictions.表 / 裏 / 矛盾
- Q5（内面に持っているもの）→ profile.memories / unspoken
- Q6（出会い + 関係性の起点）→ context、Initial Scene Anchors、current/scene.md

### relationship_spine.md

- Q1-Q9 + character.yaml → AIが構造1つ・パターン1-2個を選び、関係テーマと変化方向を生成する。
- Q6/Q7は重要素材だが、テンプレ穴埋めではなくAIが関係の起点と呼称の温度として再構成する。

### story_spine.md

- Q1-Q9 + character.yaml → AIがMain Question、Reveal Ladder、Background Truth、Pressure Direction、Heroine Tie、if ignored、Drift Guardを生成する。
- Q3/Q4/Q5は重要素材だが、Q&A本文を丸写しせず、ヒロイン固有の秘密、境界、圧として再構成する。

### protagonist.md (Wave 7)

- Q7 → 呼ばれ方セクション
- Q8 → 身体・スタイル・仕事セクション
- Q9 → Session Constraints セクション

### knowledge_state.md (Wave 8)

- Q7 → protagonist_call_name: meta
- Q8 → protagonist_gender / height / build / style / occupation: observable または meta
- Q1 → heroine_name / heroine_occupation: meta（自己紹介前）
- Q3 → heroine_visual_anchor: observable
- Q5 / story_spine → heroine_background_truth / Reveal Ladder: gm_only
- Q9 → session_constraints: meta-system
...
## downstream files のAI生成

Wave 12.2以降、profile.md / relationship_spine.md / story_spine.md の生成完了後、
`tools.session.document_generator.generate_session_documents` が current / story / lilia/main の13 downstream filesをAI駆動で生成する。
`protagonist.md` と `knowledge_state.md` もこの経路で生成し、旧Python穴埋めは使わない。

## protagonist.md の初期化

protagonist.md は session document generator が、Q7/Q8/Q9 と profile / story_spine を解釈して生成する。
...
1. `templates/session/protagonist.md` と同じ `##` 見出しを保つ。
2. 各セクションを以下の方針で埋める。
...
- 「おまかせ」の場合: profile と scene から導出する。旧プレースホルダー文字列を出さず、推論できない場合だけ短く「未確定」とする。

#### スタイル（必須）

- Q8 から服装の傾向と仕事を分解する。服装フィールドにQ8全文を丸ごと入れない。
- 「おまかせ」の場合: ヒロインの立場と出会い方から推測する。

#### Session Constraints（必須）

- Q9 に避けたい展開がある場合だけ反映する。
...
knowledge_state.md は session document generator の protagonist 系AI出力と、story_spine 由来の heroine / reveal_ladder 同期情報を統合して生成する。
...
1. `templates/session/knowledge_state.md` と同じ `##` 見出しを保つ。
2. 実データは `## knowledge_state` の fenced YAML block に1つだけ置く。
3. items セクションを以下の方針で埋める:
