...
今でも引きずっている何か。重くなくてOK。

例（構造説明のみ。literal として真似しないこと）:
- 「[大切な人から何かを継いだ]。最後に言えなかった一言がある」
- 「[前の居場所]で踏み込まれすぎて離れた。詳細は本人も整理中」
- 「[昔の共同作業]が壊れた。それ以来、[特定の感覚]を避けている」
...
- Q5の過去の傷を profile.memories / story_spine.Background Truth へ直接反映する
...
Q&Aから、ヒロインの基本、見た目、描写の縛り、表と内の差、過去の傷、最初の出会い、主人公の身体・格好・仕事、呼称を抽出する。
...
1. Main Question
   ...
   - Q5 に具体的な過去の傷がある場合、その内容を Main Question に反映する。
   - `[汎用テンプレの問い]` を名前だけ変えて使わない。

2. Background Truth
   - Q5 に過去の傷や保留がある場合、その回答を必ず骨格に含める。
   - `[汎用的な未整理事情]` のような文言で Q5 を置き換えない。

3. Reveal Ladder
   - 段階1-3 のうち、最低1つに Q3 の物的アンカーを使う。
   - 段階2 は Q4 の表と内の差がにじむものを置く。
   - 段階3 は Q5 の核心、または profile.memories / unspoken の核心に近づくものを置く。
...
- Q5 が「おまかせ」 → Q1 + Q4 から漠然とした保留や傷を仮定する。
...
- Q5（過去の傷の種）→ profile.memories / unspoken
