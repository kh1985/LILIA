...
名前、年齢、立場（職業・学校・状況）を教えてください。

例（構造説明のみ。literal として真似しないこと）:
- 「[ヒロイン名A]、[年齢または年代]、[生活上の立場]」
- 「[ヒロイン名B]、[年齢未確定]、[表向きの所属や状況]」
- 「[ヒロイン名C]、[学年や職能]、[主人公との初期距離]」
...
あなたとヒロインは、どこで・どんなふうに最初に会いますか？

例（構造説明のみ。literal として真似しないこと）:
- 「[時間帯/場所]で、[ヒロインが困っている小さな出来事]に居合わせる」
- 「[生活の場]で、[物的アンカー]をきっかけに会話が始まる」
- 「[手続き/仕事/約束の場]で、[互いの立場が少し見える状況]になる」
...
ヒロインに、毎回出てくる物的な印を1つ挙げてください。物・身体的特徴・癖、何でも構いません。
...
ヒロインが他人に見せる顔と、一人の時の顔は、何が違いますか？

例（構造説明のみ。literal として真似しないこと）:
- 「[外向きの態度]だが、[一人の時の反応]が違う」
- 「[人前で保つ顔]の裏に、[夜や帰宅後に出る癖]がある」
- 「[礼儀/冗談/無口などの表の防壁]で、[信じられないもの]を隠している」
...
ヒロインの過去に、今でも引きずっている何かがありますか？
詳細でも、ぼんやりした感触だけでも構いません。
...
あなたの身体と普段の格好を簡単に。性別、身長感、服装の傾向。

例（構造説明のみ。literal として真似しないこと）:
- 「[性別/性自認]、[身長感]、[体格]、[服装の傾向]」
- 「[小柄/高め/中くらい]、[仕事着/私服/制服など]」
- 「[見た目で観察できる範囲]だけを書く」
...
- `LILIA` を作中で名乗る名前にしない
- 年齢は成人
- Q1の立場とQ2の出会いから、今日の用事、よく触る物、初回sceneで使える具体物を導出する
- Q3の描写の縛りを、profile.mdの「描写の縛り」と opening scene の質感へ反映する
- Q4の表と内の差を、Layer 2 / Layer 3 の初期値へ反映する
- Q5の過去の傷の種を、story_spine.mdの Background Truth と Reveal Ladder の骨格へ反映する
- tone examplesを3つ出す
- personalityは行動で書く
...
Newgame Q1-Q8の回答後、初回sceneを出す前に `prompt/style_reference.md` の Light Story Reference Pass を軽く通す。
このpassは Persona Profile Generation Pass の後に行い、`profile.md` の生活、具体物、反応、矛盾を文体・温度へ接続する。

このpassは固定プロットや参照作品の再現ではない。
Q&Aから、ヒロインの基本、最初の出会い、描写の縛り、表と内の差、過去の傷、呼称、主人公の身体・スタイル、NGを抽出する。
...
Newgame Q1-Q8から裏生成した小さな出来事を、ユーザーが今触れる入口、関係に残る賭け、放置時の小さな変化へ変換する。
handlesは選択肢ではなく、自由入力の行動余地として扱う。
`story/story_deck.md` は素材・圧・未回収札、`current/event_card.md` は今のsceneで触れる可視イベントとして分ける。
Q8のNGを見て、event_cardが助け待ち一本道、明白な正解行動、重すぎる事件、甘すぎる成立済み関係へ寄っていないか確認する。
...
Newgame Q1-Q8とGM生成した保留 / 境界線から、`lilia/main/voice.md` へ呼び方、口調、沈黙、第一反応、言わない言葉を保存する。
...
1. Main Question
   ...
   - Q5 が具体的な場合、その内容を Main Question に反映する。
...
3. Reveal Ladder
   ...
   - 段階3 は Q5 の核心に近づくものを置く。

4. Drift Guard
   - Q3 の物的アンカーを必ず1項目目に含める。
   - `[Q3と無関係な汎用具体物]` を残さない（Q3 と無関係なら削除）。
...
例（構造説明のみ。literal として真似しないこと）:
- Q3 が「おまかせ」 → Q1 の立場、Q2 の出会い方から物的アンカーを発明する。
- Q5 が「おまかせ」 → Q1 + Q4 から漠然とした傷を仮定する。
...
- 後で書き換え可能。初期は仮置きでよい。
- Q1（立場）+ Q4（表と内の差）+ Q5（過去の傷）を必ず使い、名前だけ差し替えた汎用文にしない。
...
- 1項目目は必ず Q3 の物的アンカーにする。Q3 が「おまかせ」の場合だけ profile.md の「描写の縛り」か Q1/Q2 から発明した具体物を使う。
- Q3 と無関係な汎用具体物を残さない。
...
- Q1（基本: 名前・年齢・立場）→ profile の冒頭メタ情報
- Q3（描写の縛り）→ 「描写の縛り」セクション
- Q4（表と内の差）→ Layer 3（防壁）と Layer 2（核）の初期値
- Q5（過去の傷の種、断片的）→ Layer 5（核の種）
...
- Q1, Q5 → Main Question を導出
- Q5 → Background Truth の骨格
- Q2 → Reveal Ladder 段階1（最初の出会いで見える要素）
- Q3 → Drift Guard 初期項目
- Q3, Q5 → Heroine Tie
...
- Q6 → 呼ばれ方セクション
- Q7 → 身体・スタイルセクション
- Q8 → Session Constraints セクション

### knowledge_state.md (Wave 8)

- Q6 → protagonist_call_name: meta
- Q7 → protagonist_gender / height / build / style: observable
- Q1 → heroine_name / heroine_occupation: meta（自己紹介前）
- Q3 → heroine_visual_anchor: observable
- Q5 → heroine_background_truth / Reveal Ladder: gm_only
- Q8 → session_constraints: meta-system
...
- 「おまかせ」の場合: ヒロインの立場と関係から自然な呼称を選ぶ。
  - 例: 同級生→下の名前 or あだ名、職場の関係→名字さん、対立的な関係→君 or あんた

#### 身体（必須）

- Q7 から性別・身長感・体格を抽出する。
- 数値（cm）でも、抽象的（高め / 中くらい / 低め）でも可。
- 「おまかせ」の場合: 性別はランダム or 中性的、身長は中くらい、体格は普通。
...
#### protagonist 由来項目（Q6, Q7）
- protagonist_call_name (Q6) → fictional_status: meta
- protagonist_gender, height, build, style (Q7) → fictional_status: observable
