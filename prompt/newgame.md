...
- Q5の内面に持っているものを profile.memories / story_spine.Background Truth へ直接反映する
...
1. Main Question
   - Q1 + Q4 + Q5 を組み合わせてキャラ固有の問いを生成する。
   - Q5 を自動的に「傷」と断定しない。癖、特徴、職業的な残り香、葛藤、好きなものとしても扱う。
   - 5パターン（傷の治癒 / 選択 / 発見 / 変化 / 葛藤）から、Q&A全体に合う構文を選ぶ。
   - `[汎用テンプレの問い]` を名前だけ変えて使わない。

2. Background Truth
   - Q5 に内面の核、癖、職業的特徴、葛藤、過去の傷や保留がある場合、その回答を必ず骨格に含める。
   - `[汎用的な未整理事情]` のような文言で Q5 を置き換えない。

3. Reveal Ladder
   - 段階1-3 のうち、最低1つに Q3 の物的アンカーを使う。
   - 段階2 は Q4 の表と内の差がにじむものを置く。
   - 段階3 は選択した Main Question パターンに応じて、Q5 の意味、選択、変化、葛藤、または過去の核心に近づくものを置く。

4. Drift Guard
   - Q3 の物的アンカーを必ず1項目目に含める。
   - `[Q2/Q3と無関係な汎用具体物]` を残さない（Q2/Q3 と無関係なら削除）。

5. relationship_spine の「育てたいテーマ」
   - Q1 の性格、Q4 の表と内の差、Q6 の関係性の起点を反映する。
...
例（構造説明のみ。literal として真似しないこと）:
- Q3 が「おまかせ」 → profile.appearance / everyday anchors から物的アンカーを導出する。
- Q4 が「おまかせ」 → Q1 の性格と profile.reactions から表と内の差を仮定する。
- Q5 が「おまかせ」 → Q1 + Q4 から、傷に限らない内面の保留、癖、葛藤、または特徴を仮定する。
...
- 後で書き換え可能。初期は仮置きでよい。
- Q1（立場・性格）+ Q4（表と内の差）+ Q5（内面に持っているもの）を必ず参照し、名前だけ差し替えた汎用文にしない。
- Q5 をすべて「傷を抱えて扱い直す」構文へ押し込まない。
- 5パターンから合うものを選ぶ:
  - 傷の治癒: Q5 が明確な喪失、挫折、後悔の場合。
  - 選択: Q1 の立場や Q5 が、現状を続けるか変えるかのジレンマを含む場合。
  - 発見: Q5 が、自分でも意味が分かっていない癖や反応の場合。
  - 変化: 明確な傷より、出会いによる関係変化が核の場合。
  - 葛藤: やめたいがやめられない、続けたいが負担、という構造がある場合。

#### Reveal Ladder（必須、初期3段階）

- 段階1: 既にvisible（最初の出会いで見えるレベル）。Q3 の物的アンカーを優先して使う。
- 段階2: 数scene後に見えるレベル（Q4 の表と内の差、生活の細部など）。
- 段階3: 中盤以降に見えるレベル。選択した Main Question パターンに応じ、Q5 の意味、選択、変化、葛藤、または過去の核心へ近づく。
- 段階4-5は空欄のまま、進行で追加する。
- 全段階を `[pending]` 状態で初期化する。段階1だけ最初のsceneで `[in_progress]` になる場合がある。

#### Background Truth（必須、最低限の骨）

- Q5 に内面の核、癖、職業的特徴、葛藤、過去の傷や保留がある場合は必ず含める。
- 現在の言動を歪めている要因、または選択・発見・変化・葛藤の核。
- Reveal Ladder各段階の根拠になる情報。
- 詳細を完璧に埋めない。「未確定: 進行で決める」と書いてよい。

#### Pressure Direction（必須、3項目）

- ヒロインの設定と選択した Main Question パターンから、放置時に世界側で動くことを3つ抽出する。
- 傷の治癒なら過去に向き合う圧、選択なら選択を迫る圧、発見なら意味を見つける圧、変化なら関係を進める圧、葛藤なら葛藤を維持する圧にする。
...
- 1項目目は必ず Q3 の物的アンカー、または profile.md の「描写の縛り」にする。
- Q3 と無関係な汎用具体物を残さない。
...
- Q6（最初の出会い + 関係性の起点）→ 関係の起点
- Q7（呼ばれ方）→ ヒロインの呼称ルール

### story_spine.md

- Q1, Q4, Q5 → Main Question を導出
- Q5 または profile.memories / unspoken → Background Truth の骨格
- Q3 → Reveal Ladder 段階1（最初に見える物的アンカー）
- Q4 → Reveal Ladder 段階2（表と内の差）
- Q3 → Drift Guard 初期項目
- Q1, Q4, Q5, Q6 → Heroine Tie
