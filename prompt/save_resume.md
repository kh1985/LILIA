...
`next_hook` は候補であり、それだけでは active event ではない。
次回resume 1ターン目の入口にする場合は、`next_hook` だけでなく `current/scene.md`、`current/event_card.md`、`current/hotset.md` の active 部分も同じ次sceneへ揃える。
...
resume 1ターン目では、active event_card / scene / hotset / story_deck / memory / beliefs に存在しない具体手がかりを、場面進行の都合で勝手に追加しない。
特に、active stateにない小道具、書類、連絡手段、識別情報、過去の控え類を、最初からそこにあったものとして足さない。
新しい手がかりを出す場合は、active event_card の First Concrete Action に発見可能性があり、scene本文で「今発見されたもの」として扱い、Save Modeでevent_card / story_deck / memory候補へ残す。
...
候補には、主人公が新しく知ったこと、ヒロインが新しく知ったこと、双方の共有事実になったこと、疑い・仮説・belief の変化、Reveal Ladder 由来の開示状態変化、まだ `gm_only` / `meta` / `[HIDDEN until shared in scene]` として隠し続けるべき do-not-reveal 項目を含める。
ただし、確実な変化があった項目だけを `turn_update.md` に書く。
...
- 新しい事実が scene で開示された
- 観察により新しい情報が獲得された
- 主人公だけが知った事実、ヒロインだけが知った事実、双方が共有した事実を known_to で分ける
- まだ開示されていない真相や値は、do-not-reveal として hidden 状態を維持する

#### known_to の追加
- 新しい主体が情報を知った

#### 認識の変化
- 疑い、belief、仮説、誤解、確信度が変わった
- reveal 状態が変わった、または変わらず隠す必要が残った
...
- do-not-reveal 項目は本文開示のためではなく、次回も漏らさないために残す
...
resume 1ターン目では、以下に明示された具体物・連絡手段・書類・過去の控えだけを、最初から場にあるものとして扱う。
...
`story/story_deck.md` は素材棚として見る。
背景化されたeventやCandidate Next Hookを、active scene / event_card / hotset に接続していないまま、resume 1ターン目の新しい証拠として前景化しない。
これらに無い具体手がかりは、場面を進めるための便利な小道具として補わない。
出す必要がある場合は、今sceneで発見されたものとして本文中で分かるようにし、active event_cardのFirst Concrete Actionと矛盾させない。
