...
8. World Autonomy / Pressure Loop
   ...
   - ただし、親密シーンを雑な乱入で壊さない。
   - `if ignored`、visible consequence、1-3 scenes within changeを導入する。
   - 旧LIRIA `prompt/runtime.md` と `prompt/villain_engine.md` を参考にするが、初期MVPでは重い組織、勢力、幹部、クロック階層は採用しない。
   - life pressure、social pressure、relationship pressure、secret exposure pressureを、生活・信用・沈黙・約束の小さな変化として扱う。
   - Status: 未着手
