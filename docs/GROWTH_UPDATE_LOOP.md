...
scene closure後の余韻を引き延ばすためではなく、次に自然に戻れる小さな圧として残す。
`apply-turn` は `next_hook` を `current/event_card.md` の `Next Hook` と `story/story_deck.md` の `Candidate Next Hook` に反映する。
Three Hook Spineでも同じく、3本hookは3択UIではない。
active scene / event_card / hotsetへ昇格していないCandidate Hookは、resume 1ターン目のplayable入口ではない。

Three Hook Spine の状態を保存更新する場合は、Save Mode の `turn_update.md` に `## hook_updates` を明示する。
`hook_updates` がない時、`apply-turn` はhook状態を会話本文から推論して更新しない。
`hook_updates` で扱うのは、`hook_id`、`hook_type`、`status`、`current_function`、`current_question`、`exit_condition`、`next_candidate`、`Candidate Next Hooks` への候補追加など、Markdownで読める軽い状態だけにする。
Active Hook を切り替える場合は、現在の `current/event_card.md` のactive fieldsと矛盾させない。初期実装では、event_card側のActive Hook更新は明示された安全条件がある時だけ行い、重いhook engine化はしない。
Relationship Hookを好感度、攻略ルート、bondとして扱わない。
Life Hookは単独行動、遠出、同行/非同行、帰宅、食事、再訪などを関係とstoryへ戻す受け皿として扱う。
