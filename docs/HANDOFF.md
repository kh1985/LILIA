...
combat / villain / visual / manga / AI Harness は、長期ROADMAP上の参照候補であり、初期MVPやNew Session Initializationには採用しない。
...
- ROADMAP上の直前項目は New Session Initialization。現状は設計仕様とテンプレート補強が完了済み。launcher / CLIによる実生成コードは後続。
- 次の実作業は Case / Event Card Playability Gate。
- `current/event_card.md` に visible problem、first concrete action、handles 2-4、if ignored、next visible change、relationship stake を持たせる雛形は追加済み。次は旧LIRIAの Visible Request Gate / Truth Hiding Boundary / Mid-Story Activation Gate を、LILIAの小さな可プレイ性チェックとして設計する。
