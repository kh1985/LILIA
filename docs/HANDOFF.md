...
- `prompt/newgame.md` のQ&Aは Wave 10.2 時点で Q1-Q9。Wave 10 の性格/見た目/主人公仕事/interactive補足質問、Wave 10.1 の描写の縛り/表と内の差/内面に持っているものの独立質問を維持する。
...
- Q3は `profile.md` の描写の縛りと everyday anchors、Q4は `profile.contradictions`、Q5は「内面に持っているもの」として `current/story_spine.md` の Background Truth / Reveal Ladder、Q7-Q8は `current/protagonist.md` へ直結する。
...
- Q3（描写の縛り）→ profile.描写の縛り / everyday anchors、Q4（表と内の差）→ profile.contradictions、Q5（過去の傷）→ profile.memories / story_spine.Background Truth へ直接写像する。
...
- `templates/session/story/story_spine.md` に Main Question の5パターン（傷の治癒 / 選択 / 発見 / 変化 / 葛藤）と、Reveal Ladder / Background Truth / Pressure Direction の対応を追記した。
- `./lilia` の story_spine fallback 生成は、Q1/Q5 から5パターンを判定し、Main Question / Reveal Ladder / Pressure Direction / Background Truth を同じパターンで生成する。
