...
- ROADMAP上の直前項目は Launcher / CLI。最小launcherは実装済みで、prompt-only smokeも通過済み。
- 次の実作業は Launcher / CLI の最小運用確認。
- `./lilia new --prompt-only`、`./lilia list-sessions`、`./lilia resume --prompt-only` を実プレイ導線として使い、表示や引数の不足がないかを見る。
