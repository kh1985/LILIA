...
- `saves/` は `.gitignore` でgit管理外にしている。AI実行、AI Harness、自動プレイ生成、大量ログ解析、画像/漫画export、production CI はまだ入れていない。
...
- ROADMAP上の直前項目は Launcher / CLI。最小launcherは実装済みで、prompt-only smokeとUX小修正も完了済み。
- 次の実作業は Launcher / CLI の実プレイ投入確認。
- `./lilia new --prompt-only`、`./lilia list-sessions`、`./lilia resume --prompt-only` を実プレイ導線として使い、AI実行なしのmanual運用で足りるかを見る。
