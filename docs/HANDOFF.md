...
- ROADMAP上の直前項目は MVP Playtest。`new -> first scene -> save -> resume` を1周通過し、PASS with minor follow-up candidates / minor follow-up反映済み。
- 次の実作業は Launcher / CLI の最小設計確認。
- まだ launcher / CLI 実装には入らず、`new` / `resume` / `consult` / session list / prompt-only / engine fallback のうち初期MVPに必要な範囲だけを整理する。
