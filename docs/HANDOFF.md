...
- ROADMAP上の直前項目は Crisis / Combat / Ability Constraint Loop。現状はdocs正本化とテンプレート最小接続が完了済み。実生成コード、launcher / CLI、自動プレイ検証、AI Harnessは後続。
- 次の実作業は Technical + Gameplay Integrity Checks の設計正本化または最小チェック接続。
- 次は `new -> first scene -> save -> resume` の流れで、event_card、voice continuity、romance / intimacy、growth update、story accumulation、crisis ability の接続が壊れていないか確認する。
- 初期は軽い手動チェックまたは最小スクリプトに留め、AI Harness、大量ログ解析、launcher / CLI はまだ後続にする。
