...
- inner-galge: キャラ中心、hotset、Markdown運用、style defaults、romance/intimacy運用、memory model、validation、voice continuity、command導線、更新ループ、感情の骨を抽出して現在キャラへ変換する手順
...
- ROADMAP上の直前項目は Technical + Gameplay Integrity Checks。現状はdocs正本化とmanual checklist最小接続が完了済みで、最小スクリプトは現時点では不要と判断済み。
- `tests/mvp_playtest/manual_checklist.md` も追加済み。次の実作業は MVP Playtest 実施。
- MVP Playtestでは、手動チェックリストに沿って `new -> first scene -> save -> resume` を1周通し、LILIAの声、距離、event_card入口、関係、記憶、beliefs、親密境界、story accumulation、crisis / ability、growth updateが破綻していないか確認する。
- 初期は軽い手動チェックを優先し、AI Harness、大量ログ解析、launcher / CLI、production CI はまだ後続にする。
