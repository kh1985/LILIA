...
- ROADMAP上の直前項目は Technical + Gameplay Integrity Checks。現状はdocs正本化とmanual checklist最小接続が完了済み。最小スクリプト、launcher / CLI、自動プレイ検証、AI Harnessは後続。
- 次の実作業は Technical + Gameplay Integrity Checks の最小スクリプト要否確認、またはMVP Playtest準備。
- 次はファイル存在確認、見出し確認、ROADMAP / HANDOFF のNext Task簡易確認程度の最小スクリプトが必要かを確認する。不要ならMVP Playtest準備へ進む。
