...
- story function framework: `docs/specs/STORY_FUNCTION_FRAMEWORK.md`
- three hook spine: `docs/specs/THREE_HOOK_SPINE.md`
...
Three Hook Spineでは、event_cardはActive Hookだけを持つ。
Main / Relationship / Life-Explorationの3本を、毎ターンの3択UIとしてevent_cardに並べない。
Story / Relationship Accumulation では、event_card は関係の物語へ積み重なる現在点である。
event_cardでLILIAが自然に反応し、その結果が `memory`、`relationship`、`beliefs`、`voice` に残った時、story_deckやrelationship_spineの線が育つ。
...
### Next Hook Promotion Rule

Candidate Next Hook を次回sceneの入口に使う場合、`current/event_card.md` の active fields へ昇格する。
`story/story_deck.md` に候補として残しただけでは、resume 1ターン目の入口として扱わない。
Candidate Hookは、active scene / event_card / hotsetへ昇格して初めてplayableになる。

昇格時は、少なくとも以下を次scene用に揃える。
...
古いevent_cardを `current/event_card.md` の上部active stateに残したまま、末尾に `Next Hook` だけを追記しない。
残す場合は、`story/story_deck.md` の背景化したevent_card、または `archive/beats/` へ逃がす。
...
- NPCがTier条件を満たし、LILIAのmemory / relationship / beliefsに影響する入口になった。
...
親密sceneの段階、合意、境界線、aftercare保存は `docs/ROMANCE_INTIMACY_GROWTH.md` を正本とする。
外圧やstory pressureは、親密sceneをランダムに壊すためではなく、aftercare、翌朝の第一声、言い残し、後で戻る未回収札として扱う。
...
intimacy stage、consent stage、boundary stateはevent_cardではなく `relationship.md` に保存し、event_cardには今触れる入口だけを置く。
...
- 官能・親密を安全の名目で全部薄めている。
- 設定説明だけで問いや行動入口がない。
- Scene Function上の役割がなく、入口と出口で何が変わるか分からない。
...
- Scene Function Check上の役割があり、入口と出口で変わるものが見える。
- 設定説明ではなく、問いと行動入口を作っている。
- 声、呼び方、距離感に変化を残す場合は、`docs/VOICE_CONTINUITY.md` のGateに接続できる。
- 初回sceneでも重すぎない。
- 親密・官能の余地を消さず、成人、合意、相互性、境界線を守る。
- aftercareや翌朝の第一声が今触れる入口になる場合、relationship / memory / beliefsへ何を残すかが分かる。
...
- inner-galge: hotset、Markdown運用、validation、プレイヤーが触れる行動余地の足場、voice continuity、romance/intimacy
- LIRIA: event_card、case_engineの可視入口、Visible Request Gate、Truth Hiding Boundary、Mid-Story Activation Gate、save/resume方針、romance

## 13. 採用しなかったもの

- 重いcase_engine
- villain_engine / 重いorganization運用
- combat処理
- 複数ヒロイン事件構造
- 真相の全開示
- 抽象的な違和感だけで進める運用
