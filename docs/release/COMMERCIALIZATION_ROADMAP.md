...
この文書は、LILIA を小規模商用βとしてリリースするための商用化方針、MVP境界、段階計画、Go/No-Go条件を管理する release 正本である。
細かいタスクID、status、priority、Done Criteriaは `docs/release/RELEASE_WBS.md` を正本にする。
現在地と次にCodexへ投げる優先順は `PROJECT_CONTROL.md`、直近引き継ぎは `docs/HANDOFF.md` を見る。
`STATUS_DASHBOARD.html` は人間用ビューであり、正本ではない。

## Role

この文書は商用βの目的、MVP境界、作るもの / 作らないもの、Image Strategy、Phase Plan、Success Metrics、Go / No-Go、Security方針を持つ。
細かい作業表、担当、期限、status更新は `docs/release/RELEASE_WBS.md` に寄せる。
...
- WebUIでresumeできる
- 10ターン以上自然に遊べる
- WebUI上で読める通常ターン量と、プレイヤーが返しやすい Lightweight Tempo Guard がある
- Arc Closure Guard があり、良いsceneを良いところで閉じ、memory候補 / next hook / 次arc候補へ接続できる
- Save Mode / apply-turn が運用できる
- scene-tick が進む
- Three Hook Spine MVP があり、脱線してもゲームが破綻しない
- 初期storyが閉じた後も次のstory arcを生成して継続できる
- 関係変化を、数値ではなく memory / relationship / beliefs / voice / hotset の文字情報で扱える
- AI Playtest / Integrity Auditで、関係変化が早すぎる / 遅すぎる / 根拠がない状態を検出できる
- 節目画像が生成される
- 画像が同一キャラとして許容できる
- 有料β導線がある
- 利用規約がある
...
- サポート負荷
- ユーザーが忖度して本音を言わないリスク
- 1ターンで複数hookや複数質問を投げ、プレイヤーが返しづらくなるリスク
- 通常ターンがWebUIで読みにくい長さになり、会話テンポが崩れるリスク
- Lightweight Tempo Guard未整備によるゲーム体験低下
...
以下は商用βリリースに必須ではないが、忘れずに残しておく未確定議題である。
詳細な長期判断は `docs/ROADMAP.md`、βまでのタスク状態は `docs/release/RELEASE_WBS.md` を見る。
本文書には商用化観点での扱いだけ記録する。

- **テンポ管理（P-A）**: 重いチューニングは Phase 1 以降。ただしβ前は Lightweight Tempo Guard と Arc Closure Guard を P0 品質条件とし、1ターンで前景化するhookを原則1本、LILIAからの明示質問を原則1個に絞る。通常ターンの出力量、場面転換、次に返せる入口、選択肢UIの抑制、scene closure後の余韻過多、同一モチーフ反復、長文低進行は `docs/specs/PLAY_MODE_SPEC.md`、`HOOK-007`、`ARC-008〜ARC-011` を正本にする。
- **Hidden 深化ベクトル運用（P-B）**: 初期βでは本格運用しない。通常進行メーターや数値ロック解除として使わず、深い関係に到達した後の将来の質的管理候補として保持する。関係変化は `docs/RELATIONSHIP_CHANGE_AUDIT.md` に従い、文字情報で監査する。
- **深化タグ機械チェック（P-C）**: inner-galge デフォルト 14 タグ + ヒロイン追加機構。β では人力評価のみ。Phase 2 以降で機械チェック化を検討。
- **3 本フック運用（戦闘なし版、P-D）**: β前P0へ格上げ済み。詳細は `## 12. Three Hook Spine MVP` と `docs/release/RELEASE_WBS.md` の `HOOK-001〜HOOK-007` を正本にする。
...
- LILIAは小説生成ではなく、プレイヤーが返せるAI恋愛ADV / AI TRPG型体験である。文章品質が高くても、scene closure後に余韻を長く続けすぎるとWebUIゲーム体験が低下する。
- 商用βでは Arc Closure Guard を P0 品質条件とする。良いsceneは、良いところで閉じ、memory候補 / next hook / 次arc候補へ接続する。
- 「美文で引っ張る」より、「プレイヤーが次に何をしたくなるか」を優先する。
...
- 実装作業、status、Done Criteriaは `docs/release/RELEASE_WBS.md` の `HOOK-001〜HOOK-007` と `TEMPO-001〜TEMPO-005` を正本にする。
- AI Playtest側のpersona、judge、report形式は `docs/testing/AI_PLAYTEST_PLAN.md` を見る。
- このセクションは、商用βでThree Hook SpineをP0扱いにする理由と、満たすべき体験条件だけを保持する。
...
- 実装作業、status、Done Criteriaは `docs/release/RELEASE_WBS.md` の `ARC-001〜ARC-011` を正本にする。
- Long-run smokeやtraveler personaの評価設計は `docs/testing/AI_PLAYTEST_PLAN.md` を見る。
- このセクションは、初期story後もゲームが止まらないための商用方針だけを保持する。
