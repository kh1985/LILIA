...
- TEMPO-001 / TEMPO-002: passive / short-reply closure drift guardを `prompt/core.md`、`docs/specs/PLAY_MODE_SPEC.md`、AI Playtest Judgeへ追加し、passive再実行で `arc_closure_scene_progression` 5/5、segmented cycle3で4/5 PASSを確認。`tests/resume_smoke/results/2026-05-10_passive_closure_drift_fix.md` と `tests/resume_smoke/results/2026-05-10_segmented_longrun_cycle3.md` に証跡あり。ただしWBS表のDone Criteria全体の棚卸しは未実施のためstatusは `todo` 維持。
...
- AI-007: playtest run copyにcheckpoint `turn_update` を本適用し、`validate-session` / `resume --prompt-only` / `resume --run` の再開品質を確認済み。さらに適用済みrun copyをsourceにした2区間目AI Playtestと、Segment 2 checkpoint適用後のcycle3 resume / next10 PASSまで確認。`tests/resume_smoke/results/2026-05-10_checkpoint_apply_smoke.md`、`tests/resume_smoke/results/2026-05-10_checkpoint_quality.md`、`tests/resume_smoke/results/2026-05-10_segmented_longrun.md`、`tests/resume_smoke/results/2026-05-10_segmented_longrun_cycle3.md` に証跡あり。source `saves/` ではなくrun copy限定のため `review`。
...
- ARC-006: segmented long-run continuationとして、normal 10ターンcheckpoint、run copyへのcheckpoint本適用、validate/resume確認、適用済みrun copyからの2区間目10ターンAI Playtest、さらにcycle3 resume / next10 PASSまで成功。Segment 2のclosure drift警告はcycle3で改善し、`arc_closure_scene_progression` は4/5。`tests/resume_smoke/results/2026-05-10_segmented_longrun.md`、`tests/resume_smoke/results/2026-05-10_segmented_longrun_cycle2.md`、`tests/resume_smoke/results/2026-05-10_segmented_longrun_cycle3.md` に証跡あり。ただしDone Criteriaの100ターン級には未到達のためstatusは `todo` 維持。
...
- ARC-010: AI Playtest checkpointがclosure候補を `checkpoint_turn_update.md` の `## next_hook` と安全側の `## hook_updates` candidateへ接続し、`apply-turn --dry-run` で確認できる。Arc Closure Drift対策後のnormal 10ターン再実行、passive / short-reply closure drift対策後のpassive再実行、Segment 2 checkpoint適用後のcycle3 resume / next10はいずれもPASS。`tests/resume_smoke/results/2026-05-10_closure_to_hook_checkpoint.md`、`tests/resume_smoke/results/2026-05-10_closure_drift_fix.md`、`tests/resume_smoke/results/2026-05-10_passive_closure_drift_fix.md`、`tests/resume_smoke/results/2026-05-10_segmented_longrun_cycle3.md` に証跡あり。ただしruntimeのclosure-to-hook active state生成はrun copy / checkpoint候補経由の確認に留まるため `review`。
- ARC-011: normal / wanderer / passiveのlong-run checkpoint smokeを記録し、normalはclosure drift対策後の再実行でPASS。passive再実行はPASSし、Segment 2のclosure drift警告はcycle3 next10でPASSへ改善。`tests/resume_smoke/results/2026-05-10_longrun_normal_wanderer.md`、`tests/resume_smoke/results/2026-05-10_closure_drift_fix.md`、`tests/resume_smoke/results/2026-05-10_passive_recheck.md`、`tests/resume_smoke/results/2026-05-10_passive_closure_drift_fix.md`、`tests/resume_smoke/results/2026-05-10_segmented_longrun_cycle3.md` に証跡あり。30〜40turn級ログはcheckpoint分断とrun copy連鎖での確認に留まるため `review`。
