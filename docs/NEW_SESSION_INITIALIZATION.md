...
1. `prompt/newgame.md` のSimple Q1-Q5を聞く。
...
9. `profile.md` の `Initial Scene Anchors` / `context` / `unspoken` / `everyday anchors` から、`current/scene.md`、`current/event_card.md`、`story/story_deck.md`、`story/relationship_spine.md`、`current/hotset.md`、`lilia/main/*` を初期化する。
   ...
   Simple Q1-Q5ではプレイヤーに場所、今日の保留、境界線、初回イベントを直接聞かず、ヒロイン像、初期関係、体験の味、NG、職業・生活からGM / Story側で裏生成する。
...
| Q3. どんな味の話にしたいか | `story/story_deck.md`, `story/relationship_spine.md`, `current/event_card.md`, `style/reference.md`, `style/rules.md` |
...
| --- | --- | --- |
| 初回から演じるためのPersona Profile | `lilia/main/profile.md` | Simple Q1-Q5、character YAML、GM生成した生活/保留/境界線/eventから作る。完成済み攻略キャラカードにせず、生活、行動、矛盾、反応、禁忌を初回scene前に1枚へまとめる |
...
| Q5の職業・生活 | `lilia/main/profile.md`, `current/scene.md`, `current/event_card.md`, `story/story_deck.md` | 出会いの場所を直接聞かず、今日の用事、よく触る物、初回sceneで使える具体物へ導出する |
...
台詞テンプレではなく、声の方向性を置く。
例文を固定台詞として保存しない。
Simple Q1-Q5とGM生成した保留 / 境界線から初期voice baselineを作るが、場所や初回eventをプレイヤーに直接聞かず、ユーザー回答から抽象軸だけを取り出す。
