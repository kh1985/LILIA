...
1. `prompt/newgame.md` の Newgame Q1-Q9 を聞く。
...
6. Persona Profile Generation を行う。
   ...
   4. launcher が `tools.character.profile_generator.generate_profile_document(answers=..., character_yaml=..., engine=...)` を呼び、AI-driven `profile.md` を生成する。
   5. `lilia/main/profile.md` を生成する。
      `profile.md` の `name:` は作中で名乗る個体名にする。作品名・存在カテゴリとしての `LILIA` を作中名にしない。
   6. profile generator は検証に失敗した場合 `ProfileGenerationError` を投げ、launcher は hard-fail する。壊れた `profile.md` は保存しない。
   7. apply-newgame の log には `[profile] generated via`、`[profile] validation`、`retry_count`、`sections_count` を残す。
   8. 必要なら source/debug 用に `lilia/main/character.yaml` または `archive/checkpoints/` へYAMLを置く。
7. `profile.md` の `name:` から個体名を抽出し、`session.json` の `lilia_name` / `lilia_display_name` へ保存する。
8. `current/story_spine.md` と `story/relationship_spine.md` は、生成済みの character YAML とQ&Aをもとに `tools/story/spine_generator.py` がAI駆動で生成する。Pythonの穴埋めテンプレートは使わない。
9. 両spineの生成と検証が通った後、launcher は `tools.session.document_generator.generate_session_documents` を呼び、`profile.md`、character YAML、Q&A、`current/story_spine.md`、`story/relationship_spine.md` を入力して13 downstream filesを生成する。
   `render_profile_initialized_documents` / `render_protagonist_document` / `render_knowledge_state_document` / `render_newgame_documents` は削除済みで、新規初期反映ではPython穴埋め経路を使わない。
10. session document generator は `profile.md` の `Initial Scene Anchors` / `context` / `unspoken` / `everyday anchors` と両spineから、`current/scene.md`、`current/event_card.md`、`story/story_deck.md`、`current/hotset.md`、`current/protagonist.md`、`current/knowledge_state.md`、`lilia/main/*` を初期化する。
...
このうち `lilia/main/profile.md` と `lilia/main/character.yaml` は profile / character 生成段で作る。
`current/story_spine.md` と `story/relationship_spine.md` は先に `tools/story/spine_generator.py` が生成する。
13 downstream files（`current/scene.md`、`current/hotset.md`、`current/event_card.md`、`current/relationship_overview.md`、`current/protagonist.md`、`current/knowledge_state.md`、`lilia/main/core.md`、`lilia/main/voice.md`、`lilia/main/state.md`、`lilia/main/relationship.md`、`lilia/main/memory.md`、`lilia/main/beliefs.md`、`story/story_deck.md`）は、spine生成後に `tools.session.document_generator.generate_session_documents` が生成する。
...
Q&A結果は、`prompt/newgame.md` の現行 Newgame Q1-Q9 を正本として、以下のように分けて保存する。
旧Q番号（例: Q2=出会い、Q5=過去の傷、Q6=呼ばれ方、Q8=避けたいもの）は使用しない。

| 初期質問 | 主な保存先 |
| --- | --- |
| Q1. ヒロインの基本 | `lilia/main/profile.md`, `lilia/main/core.md`, `lilia/main/voice.md`, `current/story_spine.md` の Main Question |
| Q2. ヒロインの見た目 | `lilia/main/profile.md` の appearance / body / outfit, `style/defaults/heroine_appearance.md`, `prompt/opening_scene.md` の視点整合 |
| Q3. 描写の縛り | `lilia/main/profile.md` の描写の縛り, `current/story_spine.md` の Drift Guard / Heroine Tie, `current/event_card.md` の具体物 |
| Q4. 表と内の差 | `lilia/main/profile.md` の contradictions / Layer 2 / Layer 3, `lilia/main/state.md`, `lilia/main/beliefs.md`, `story/relationship_spine.md` |
| Q5. 内面に持っているもの | `current/story_spine.md` の Background Truth / Main Question / Reveal Ladder, `lilia/main/profile.md` の memories / unspoken / Layer 5, `story/story_deck.md` |
| Q6. 最初の出会い + 関係性の起点 | `current/scene.md`, `current/event_card.md`, `current/relationship_overview.md`, `story/relationship_spine.md`, `current/story_spine.md` の Reveal Ladder 段階1 |
| Q7. ヒロインからの呼ばれ方 | `current/protagonist.md`, `lilia/main/voice.md`, `current/relationship_overview.md` |
| Q8. 主人公の身体・格好・仕事 | `current/protagonist.md` の身体 / スタイル / 仕事, `current/knowledge_state.md`, `prompt/opening_scene.md` と `style/defaults/heroine_appearance.md` の視点整合 |
| Q9. 避けたい展開・苦手なノリ | `current/protagonist.md` の Session Constraints, `style/rules.md`, `lilia/main/profile.md` の forbidden, First Scene Quality Gate判断 |

| Q&Aから抽出するもの | 主な保存先 | 保存方針 |
| --- | --- | --- |
| 初回から演じるためのPersona Profile | `lilia/main/profile.md` | Q1-Q9、character YAML、AI profile generatorが生成した生活/保留/境界線/eventから作る。完成済み攻略キャラカードにせず、生活、行動、矛盾、反応、禁忌を初回scene前に1枚へまとめる |
| ヒロイン像 | `lilia/main/profile.md`, `lilia/main/voice.md`, `lilia/main/state.md`, `style/reference.md` | Q1とQ2を中心に初回sceneの見え方として保存し、LILIAそのものをユーザー回答で全置換しない |
| 現在の関係位置 | `current/relationship_overview.md`, `lilia/main/relationship.md`, `current/scene.md` | Q6の出会いと関係性の起点を関係の温度として保存し、好意や恋愛成立を確定しない |
| LILIAの人格核 | `lilia/main/core.md`, `lilia/main/voice.md`, `lilia/main/state.md` | Q1、Q4、Q5から固有の価値観、弱さ、距離の取り方として必要最小限だけ保存する |
| LILIAの声、呼び方、第一反応 | `lilia/main/voice.md`, `current/relationship_overview.md`, `current/hotset.md` | Q7を呼び方の初期根拠にする。固定台詞ではなく、呼び方、沈黙、言わない言葉、変わってよい揺れとして保存する |
...
| Q2の見た目 | `lilia/main/profile.md`, `style/defaults/heroine_appearance.md`, `prompt/opening_scene.md` | 髪、目、体型、服装の雰囲気を、初登場描写と視点整合に使う。見た目だけで人格を確定しない |
| Q3の描写の縛り | `lilia/main/profile.md`, `current/story_spine.md`, `style/defaults/heroine_appearance.md` | 半永続の質感として保存し、登場描写で角度を変えて繰り返す |
| Q5の内面に持っているもの | `current/story_spine.md`, `lilia/main/profile.md`, `story/story_deck.md` | 傷に限定しない。癖、習慣、こだわり、職業的特徴、葛藤、好きなもの、保留として扱い、必要に応じてBackground TruthやReveal Ladderへ解釈反映する |
| Q6の最初の出会い + 関係性の起点 | `current/scene.md`, `current/event_card.md`, `current/relationship_overview.md`, `story/relationship_spine.md` | 初回sceneの場所、距離、関係の温度、最初に触れる可視入口へ反映する |
| Q7の呼ばれ方 | `current/protagonist.md`, `lilia/main/voice.md`, `current/knowledge_state.md` | ヒロインがプレイヤーをどう呼ぶかの初期根拠にする。理由なく後から変えない |
| Q8の主人公情報 | `current/protagonist.md`, `current/knowledge_state.md`, `prompt/opening_scene.md` | 身体、服装、仕事だけを保存する。主人公の内面情報は保存しない |
...
| Q9のNG・避けたい展開・苦手なノリ | `style/rules.md`, First Scene Quality Gate, `current/protagonist.md`, `current/event_card.md` | sceneを弱くするためではなく、事故を避けるための制約として扱う |
...
Wave 12.1以降、`profile.md` は character YAML とQ&Aを入力にした AI-driven profile generator が作る。
launcher内の旧Python変換器や穴埋めfallbackを正本にしない。
基礎情報、tone、personality、values、everyday anchors、memories、contradictions、unspoken、reactions、forbidden、context、fixed memory、5層構造、relationship progression、latent jealousy slot、dormant ability slotをAI生成し、検証を通ったものだけ保存する。
Wave 12.2以降、profile生成直後にdownstream docsを作らない。先に `current/story_spine.md` / `story/relationship_spine.md` をAI生成し、その後 session document generator が `profile.md` と両spineを合わせて downstream docs へ分解する。
...
profileを毎ターン肥大化させない。
`Initial Scene Anchors` は、両spine生成後に `current/scene.md`、`current/event_card.md`、`story/story_deck.md`、`current/hotset.md`、`lilia/main/*` へ分解し、初回sceneの現在地、可視問題、次beat、未回収札、初期voice / state / relationship / memory / beliefsとして使う。
...
台詞テンプレではなく、声の方向性を置く。
例文を固定台詞として保存しない。
Q1-Q9とGM生成した保留 / 境界線から初期voice baselineを作る。
呼び方はQ7から取り、固定台詞ではなく声の一貫性として扱う。
