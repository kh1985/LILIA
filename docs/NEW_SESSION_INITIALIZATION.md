...
6. Persona Profile Generation を行う。
   1. Q&A回答を character system 用の自然言語指示へ変換する。
   ...
   3. YAMLを検証する。
   4. YAMLを LILIA Persona Profile へ変換する。
   5. `lilia/main/profile.md` を生成する。
      `profile.md` の `name:` は作中で名乗る個体名にする。作品名・存在カテゴリとしての `LILIA` を作中名にしない。
   6. 必要なら source/debug 用に `lilia/main/profile.yaml` または `archive/checkpoints/` へYAMLを置く。
...
9. `profile.md` の `Initial Scene Anchors` / `context` / `unspoken` / `everyday anchors` から、`current/scene.md`、`current/event_card.md`、`story/story_deck.md`、`story/relationship_spine.md`、`current/hotset.md`、`lilia/main/*` を初期化する。
   `current/event_card.md` には Scene Exit / Next Beat を置き、3-5ターン以内に次beatへ移れる入口を残す。
10. `lilia/main/*` を人格核、声、状態、関係、記憶、認識に分けて初期化する。
   `lilia/main/voice.md` には、固定台詞ではなく、呼び方、声の基準、変わってよい揺れ、第一反応の方向を置く。
11. `current/*` を初回scene前でもresume可能な最小状態にする。
...
12. `story/*` に関係テーマ、初回の小さな出来事、未回収札を分けて保存する。
13. 初回scene前に Light Story Reference Pass を一度だけ軽く通す。
14. `style/reference.md` と `style/rules.md` に、本文ではなく表現軸とsession固有ルールだけを保存する。
15. `current/scene.md`、`current/event_card.md`、`story/story_deck.md` を整える。
16. `current/hotset.md` を、再開直後に温度が落ちない短いキャッシュとして再生成する。
17. first scene前に必ず `lilia/main/profile.md` と current/story/hotset 初期状態を読み、profileにある具体物、職能、生活、反応、矛盾、禁忌を使って初回sceneを開始する。名乗る場合は `lilia_display_name` / `lilia_name` を使い、`LILIA` を作中名にしない。
...
| --- | --- | --- |
| 初回から演じるためのPersona Profile | `lilia/main/profile.md` | Q1-Q7すべてとcharacter YAMLから作る。完成済み攻略キャラカードにせず、生活、行動、矛盾、反応、禁忌を初回scene前に1枚へまとめる |
| 初回に表へ出るLILIAの側面 | `lilia/main/voice.md`, `lilia/main/state.md`, `style/reference.md` | 初回sceneの見え方として保存し、LILIAそのものをユーザー回答で全置換しない |
...
| 今日だけの小さな保留 | `lilia/main/state.md`, `lilia/main/beliefs.md`, `current/hotset.md`, `story/relationship_spine.md` | 重い秘密や過去設定にせず、今日すぐには言わない揺れとして保存する |
| ユーザーに許されている距離 | `lilia/main/relationship.md`, `current/relationship_overview.md`, `lilia/main/voice.md`, `current/event_card.md` | してよいことと、踏み込みすぎた時に引く境界として保存する |
| 初回の小さな出来事 | `current/event_card.md`, `story/story_deck.md`, `current/scene.md` | 事件ではなく、関係が少し動く入口にする |
| 初回sceneで避けたいこと | `style/rules.md`, First Scene Quality Gate, `current/event_card.md` | sceneを弱くするためではなく、事故を避けるための制約として扱う |
...
台詞テンプレではなく、声の方向性を置く。
例文を固定台詞として保存しない。
Q1、Q4、Q5から初期voice baselineを作るが、新しい質問を増やさず、ユーザー回答から抽象軸だけを取り出す。
