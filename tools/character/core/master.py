"""
Claude CLI bridge for minimal character YAML generation.
...
def generate_characters(instruction: str) -> list[CharacterSheet]:
    ...
    try:
        ...
        raise RuntimeError(
            "claude command was not found. Install/login to Claude CLI, or use the "
            "fallback instruction in prompt/newgame.md to generate the same schema manually."
        ) from exc
    ...
    if not raw_dicts:
        raise ValueError("no YAML block with a character name was found in Claude output")
...
def generate_character(instruction: str) -> CharacterSheet:
    return generate_characters(instruction)[0]
