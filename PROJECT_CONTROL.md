...
β前は `RELEASE_WBS.md` のP0をタスク正本にする。
...
- `COMMERCIALIZATION_ROADMAP.md` には商用βスコープがかなり書かれているが、`RELEASE_WBS.md` の一部タスクはまだtodoである。次回以降、仕様記述済みと実装済みを分けて棚卸しする。
- `AI_PLAYTEST_PLAN.md` は大量ベンチを非目標にしている。一方、WBSには100ターン級smokeがある。これは常時大量ベンチではなく、β前の軽量release smokeとして扱う。
...
- `RELEASE_WBS.md`: βまでのタスク、優先度、状態、完了条件の正本。
- `COMMERCIALIZATION_ROADMAP.md`: 商用MVP境界、Go / No-Go、画像戦略、安全原則、段階計画の正本。
- `PLAY_MODE_SPEC.md`: Play Mode本文品質、Lightweight Tempo Guard、Arc Closure Guardの正本。
- `AI_PLAYTEST_PLAN.md`: AI Playtestのpersona、loop、judge、report形式の正本。
- `IMPLEMENTATION_HISTORY.md`: 完了済みWaveと実装履歴の台帳。現在ステータスや次タスクの正本にはしない。
...
- 開発再開 / 引き継ぎ: `docs/HANDOFF.md`, `docs/ROADMAP.md`, `RELEASE_WBS.md`
- 商用β判断: `COMMERCIALIZATION_ROADMAP.md`, `RELEASE_WBS.md`, `AI_PLAYTEST_PLAN.md`
- Play Mode品質: `PLAY_MODE_SPEC.md`, `prompt/core.md`, `docs/EVENT_CARD_PLAYABILITY.md`, `docs/VOICE_CONTINUITY.md`
...
- `RELEASE_WBS.md`: βまでのタスク正本。ID、優先度、状態、完了条件はここを更新する。
...
1. `RELEASE_WBS.md` のstatus棚卸し。既に仕様が書かれている `AI-001` / `AI-002` / `TEMPO-001` / `ARC-008` / `STORY-005` / `STORY-006` などを、仕様done・実装todoに分けて整理する。
2. `docs/HANDOFF.md` の圧縮。直近作業、次タスク、注意点に絞り、古いWave履歴は `IMPLEMENTATION_HISTORY.md` へ寄せる方針を作る。
...
5. Three Hook Spine MVPの最小実装範囲を確定する。`HOOK-001` から始め、既存 `PLAY_MODE_SPEC.md` と `STORY_RELATIONSHIP_ACCUMULATION.md` を正本として使う。
6. `STATUS_DASHBOARD.html` を「snapshot / human view」と明記するか、WBSから生成する運用に寄せる。

## archive候補

今回は安全のため、移動・削除はしない。
以下は将来の統合・移動候補であり、現時点では参照パスを壊さない。
...
- `STATUS_DASHBOARD.html`: 正本ではなくスナップショットビュー。古い表示とWBSの集計ズレが出るため、archive化または生成ビュー化候補。
- `docs/HANDOFF.md` の古いWave履歴: `IMPLEMENTATION_HISTORY.md` へ統合候補。
- `docs/ROADMAP.md` の完了Wave詳細: `IMPLEMENTATION_HISTORY.md` へ統合候補。ただしROADMAP本体は正本として残す。
- `IMPLEMENTATION_HISTORY.md` のPending / Commercialization Gap: `docs/ROADMAP.md` / `RELEASE_WBS.md` と重複するため、履歴台帳に純化する候補。

## 既知の重複・ズレ

- `STATUS_DASHBOARD.html` の集計は `RELEASE_WBS.md` とズレる可能性がある。Dashboardを正本にしない。
- `RELEASE_WBS.md` の一部todoは、仕様文書上は既に定義済みのものを含む。次回棚卸しで、仕様doneと実装doneを分ける。
