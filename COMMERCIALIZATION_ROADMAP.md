...
- **テンポ管理（P-A）**: 重いチューニングは Phase 1 以降。ただしβ前は Lightweight Tempo Guard と Arc Closure Guard を P0 品質条件とし、1ターンで前景化するhookを原則1本、LILIAからの明示質問を原則1個に絞る。通常ターンの出力量、場面転換、次に返せる入口、選択肢UIの抑制、scene closure後の余韻過多、同一モチーフ反復、長文低進行は `PLAY_MODE_SPEC.md`、`HOOK-007`、`ARC-008〜ARC-011` を正本にする。
